...
class ConfigParser:
    ...
    def organise_config_info(self, checked_config):
        ...
        if len(checked_config) != 3:
            raise Exception("Error: Invalid config header. There must be one line for each of 'router-id', 'input-ports' and 'outputs'")
        ...
        for config, line_num in checked_config:
            ...
            elif config[0] == 'outputs':
                outputs_data = config
                outputs_line = line_num
            else:
                raise Exception("Error: Invalid config header. There must be one line for each of 'router-id', 'input-ports' and 'output-ports'")

        return (router_id_data, router_id_line, input_ports_data, input_ports_line, outputs_data, outputs_line)
    ...
    def check_config_outputs(self, outputs_data, outputs_line):
        ...
        if len(outputs_data) >= 2:
            for port_index in range(1, len(outputs_data)):
                port_data = outputs_data[port_index].split("-")
    ...
    def read_config_file(self, filename):
        ...
        router_id_data, router_id_line, input_ports_data, input_ports_line, outputs_data, outputs_line = config_data
